...
跨场景只允许 mask 实际数值摘要和 mask 生成器运行时参数变化。CSV/SVG
固定导出语义路径长度、风险区穿越长度、最小语义间距和规划时间的均值及总体
标准差；manifest 保存场景摘要、原始输入和成果文件的路径与散列，作为从论文
图表回溯到原始 JSON 的审计链。建议正式实验成果输出到独立归档目录，不提交
临时 Pilot 数据到代码仓库。
