...
std::string awake_word = "请讲。";
std::string thinking = "稍等，正在思索中。 ";
std::string g_current_awake_sid; // 当前唤醒反馈的SID
std::string RECORD_DEVICE_NAME;
double VAD_THRESHOLD = 0.7;
...
std::atomic<std::uint64_t> g_wake_generation{0};
std::atomic<int> g_max_vad_volume{0};
        ...
        case AIUIConstant::EVENT_VAD: {
            ...
            switch (event.getArg1()) {
                case AIUIConstant::VAD_BOS_TIMEOUT: {
                    alsaStop();
                    std::cout << "EVENT_VAD: VAD_BOS_TIMEOUT, max_vol="
                              << g_max_vad_volume.load() << std::endl;
                    startTTS("没有听到，请重新唤醒后再说一次。", "vad_retry");
                } break;
                ...
                case AIUIConstant::VAD_EOS: {
                    alsaStop();
                    std::cout << "EVENT_VAD: EOS, max_vol="
                              << g_max_vad_volume.load() << std::endl;
                } break;

                // 音量，arg2为音量级别（0-30）
                case AIUIConstant::VAD_VOL: {
                    int current_max = g_max_vad_volume.load();
                    while (event.getArg2() > current_max &&
                           !g_max_vad_volume.compare_exchange_weak(
                               current_max, event.getArg2())) {
                    }
                } break;
            }
        } break;
...
void createAgent(bool more = true, const char* cfgPath = CFG_FILE_PATH)
{
    ...
    if (reader.parse(aiuiParams, paramJson, false)) {
        paramJson["vad"]["threshold"] = std::to_string(VAD_THRESHOLD);
        std::cout << "Applied VAD threshold: " << VAD_THRESHOLD << std::endl;
        if (more) {
...
void playAwakeFeedbackAndWait() {
    // 播放唤醒反馈
    startTTS(awake_word, "wake_feedback");
    waitForTtsComplete(1500);
}
...
void AIUI_Node::awake_Callback(std_msgs::msg::Int8::SharedPtr msg){
    awake_flag = msg->data;
    const std::uint64_t wake_generation = ++g_wake_generation;
    const bool was_recording = record_status;
    g_max_vad_volume.store(0);
    ...
    if (awake_flag == 1) {
        ...
        if (g_tts_is_playing || play_status) {
            cancelTTS();
            int wait_count = 0;
            while (g_tts_cancelling && wait_count < 20) {
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
                wait_count++;
            }
        }
        ...
        // 每次硬件唤醒都重置AIUI会话，避免长时间运行后仍停留在旧会话状态。
        wakeup();

        // 录音期间再次唤醒时直接重启本轮录音，避免重复等待云端唤醒提示音。
        if (was_recording) {
            if (audioListeningThread) {
            ...
            alsaStart();
            std::cout << "检测到重复唤醒，录音已重新开始，请说话" << std::endl;
            return;
        }

        std::thread([wake_generation]() {
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
            if (wake_generation != g_wake_generation.load()) return;
            playAwakeFeedbackAndWait();
            if (wake_generation != g_wake_generation.load()) return;
            if (audioListeningThread) {
                audioListeningThread->resetAudioDevice();
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
...
AIUI_Node::AIUI_Node(const std::string &node_name,
    ...
    this->declare_parameter<std::string>("record_device_name","default");
    this->declare_parameter<double>("vad_threshold", 0.7);

    this->get_parameter("record_device_name",RECORD_DEVICE_NAME);
    this->get_parameter("vad_threshold", VAD_THRESHOLD);
